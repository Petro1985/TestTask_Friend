...
public class UserRepository : IUserRepository
{
    ...
    public async Task<UserEntity?> GetByLogin(string login, CancellationToken cancellationToken)
    {
        return await _dbContext.Users.FirstOrDefaultAsync(user => user.Login == login, cancellationToken);
    }

    public async Task<int> Set(UserEntity user, CancellationToken cancellationToken)
    {
